...
## Fitting model: Elastic Net, Classification Tree, Random forest, and Boosted tree models

In this section, I fit the data to four models: Elastic Net, Classification Tree, Random forest, and Boosted tree models, and choose which model shows the highest ROC_AUC of prediction. 
...
Finally, to compare with the other model, I calculate the overall ROC_AUC on the training set. The ROC_AUC from the elastic net model is 0.6435718.
...
To compare with the other models, I derive the ROC_AUC based on the training model. The ROC_AUC from the classification tree model is 0.562356. 
...
Here, I apply random forests. First, I set up the specification including the tuning process for mtry.
...
Next, I set up the workflow adding the model. 
...
grid3 <- grid_regular(mtry(range = c(1, 5)), levels = c(penalty = 5))
```

Now I should show which values of mtry appear to produce the highest ROC_AUC.
...
save(tune_res3, file="model3.rda")
```
...
rf_final <- finalize_workflow(rf_wf, best3)
```

```{r, eval=F}
rf_final_fit <- fit(rf_final, data = data_train)
save(rf_final_fit, file="model3fit.rda")
```
...
As the last model, I use Boosted tree model. I set up the specification including the tuning process for tree. I did not tune tree depth since it takes too long time and set it as 4.
...
grid4 <- grid_regular(penalty(range = c(1000, 3000)), levels = c(penalty = 3))
```
...
The best model that shows the highest ROC_AUC is . Therefore, I fit this model to test set.
```{r}
...
In addition, I can draw a heatmap.
...
# Conclusion
Throughout the research, the best model to predict a child's health status using information about family background is a random forest, but it was not perfect.
